...
> Caching contracts and interfaces for the Muonroi ecosystem.
...
The `Muonroi.Caching.Abstractions` package provides fundamental building blocks for modern .NET applications. It is part of the Muonroi ecosystem, designed to accelerate enterprise software development by providing robust, scalable, and highly cohesive components. 
...
### + Muonroi.Caching.Memory → In-process implementation behind the ICacheService contract

Provides a fast, local memory cache that strictly adheres to the caching abstractions.

```csharp
builder.Services.AddMuonroiMemoryCache();
// Inject ICacheService everywhere
```

### + Muonroi.Caching.Redis → Distributed implementation; swap without changing your code

Move from local memory to distributed Redis caching just by changing the DI registration. Application code relying on `ICacheService` stays untouched.

```csharp
builder.Services.AddMuonroiRedisCache(options => options.Configuration = "localhost:6379");
```

### + Muonroi.Tenancy → ITenantCacheKeyBuilder generates isolated keys per-tenant automatically

Ensures that caching does not leak data across tenants by namespacing cache keys.

```csharp
var safeKey = tenantCacheKeyBuilder.BuildKey("UserProfile:123");
// safeKey -> "tenant-A:UserProfile:123"
```
...
```csharp
builder.Services
    .AddMuonroiCachingCore(config)
    .AddMuonroiRedisCache(config)
    .AddMuonroiTenantCacheKeyBuilder();
```
...
```powershell
Install-Package Muonroi.Caching.Abstractions
```

Alternatively, add it directly to your `.csproj`:

```xml
<PackageReference Include="Muonroi.Caching.Abstractions" Version="1.0.0" />
```

## Quick Start

Here is a minimal, complete working example of how to configure and use `Muonroi.Caching.Abstractions` in your application.
...
builder.Services.AddMuonroiCachingAbstractions(options => 
{
...
namespace SampleApp.Services
{
    public class MyBusinessService
    {
        ...
        {
            _logger.LogInformation("Executing process using Muonroi.Caching.Abstractions components.");
...
```csharp
builder.Services.Configure<CachingAbstractionsOptions>(
    builder.Configuration.GetSection("Muonroi_CachingAbstractions"));
```
...
- `IMuonroiCachingAbstractionsService`: The primary contract for interacting with this package. Provides methods to execute subsystem logic.
- `CachingAbstractionsOptions`: Configuration model representing the options available for tuning the subsystem.
...
- `muonroi.cachingabstractions.operations.count`: Total operations performed.
- `muonroi.cachingabstractions.operations.duration`: Histogram of operation latencies in milliseconds.
- `muonroi.cachingabstractions.operations.errors`: Counter for exceptions and failures.

### Traces

Traces are emitted under the activity source name `Muonroi.Caching.Abstractions`. Span names follow semantic conventions and typically include the operation name.
