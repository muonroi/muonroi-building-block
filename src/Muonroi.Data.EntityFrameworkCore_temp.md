...
> Works great standalone. Becomes **significantly more powerful** when combined with other Muonroi packages.

### + Tenancy.Core → Automatic Per-Tenant Data Isolation
When `MDbContext` detects an active `ITenantContext`, all `IQueryable<T>` calls for `ITenantScoped` entities are automatically filtered by `TenantId` — no manual `Where(x => x.TenantId == tenantId)` needed anywhere:
```csharp
builder.Services
    .AddMDbContext<AppDbContext>(config)
    .AddTenantContext(config); // tenant filter injected automatically
```

### + Messaging.MassTransit → Transactional Outbox Pattern
`SaveChangesAsync()` raises domain events into an `IOutboxRepository`. MassTransit's `OutboxRelayBackgroundService` picks them up and delivers them reliably — no event loss even if the broker is temporarily unavailable:
```csharp
builder.Services
    .AddMDbContext<AppDbContext>(config)
    .AddMassTransit(x => x.AddMuonroiOutbox<AppDbContext>());
```

### + Diagnostics → Traced Database Operations
Every `SaveChangesAsync()` and query becomes a child trace node. You see exact SQL, duration, and tenant context in your trace viewer without any extra instrumentation code.

### + RuleEngine.Core → Rule-Validated Mutations
Combine with `Mediator`'s `MRuleEngineBehavior` so domain rules run *before* `SaveChangesAsync()` — the database is never touched if rules fail.

### Full Production Data Stack
```csharp
builder.Services
    .AddMDbContext<AppDbContext>(config)           // EF Core + audit + soft-delete
    .AddTenantContext(config)                      // per-tenant query filter
    .AddMDiagnostics(config)                       // traced DB operations
    .AddMassTransit(x => x.AddMuonroiOutbox<AppDbContext>()); // reliable events
```

## Samples
- [`Quickstart.Data.EntityFrameworkCore`](../../samples/Quickstart.Data.EntityFrameworkCore)
- [`Quickstart.Data.Events`](../../samples/Quickstart.Data.Events)
