...
{
    private static readonly HashSet<string> _reservedPathPrefixes = new(StringComparer.OrdinalIgnoreCase)
    {
        "api",
        "swagger",
        "health",
    ...
    private static string? ResolveFromPath(string? path)
    {
        ...
        // Common API routes should not be interpreted as tenant ids.
        ...
        if (_reservedPathPrefixes.Contains(segments[0]))
        {
            return null;
        }
