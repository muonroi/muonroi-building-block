...
> Data layer abstractions, defining the repository and unit of work patterns.
...
The `Muonroi.Data.Abstractions` package provides fundamental building blocks for modern .NET applications. It is part of the Muonroi ecosystem, designed to accelerate enterprise software development by providing robust, scalable, and highly cohesive components. 
...
### + Muonroi.Data.EntityFrameworkCore → EF implements the IRepository<T>, IUnitOfWork contracts

Enables switching ORMs seamlessly. Use EF Core for complex writes and relationships.

```csharp
builder.Services.AddMuonroiEfCoreRepositories();
```

### + Muonroi.Data.Dapper → Dapper implements the IQueryRepository<T> read-side contracts

Provides high-performance reads by bypassing EF Core's change tracker.

```csharp
builder.Services.AddMuonroiDapperQueries();
```

### + Muonroi.Tenancy → All repository implementations scope queries by TenantId

Automatically appends tenant filters to all queries without developer intervention.

```csharp
var data = await repository.GetAllAsync(); // Automatically filtered!
```

### + Muonroi.Messaging → IUnitOfWork.CommitAsync() publishes domain events via outbox

Guarantees that database changes and message publications are atomically bound.

```csharp
unitOfWork.PublishEvent(new DomainEvent());
await unitOfWork.CommitAsync(); // Saves DB and commits to Outbox
```
...
```csharp
builder.Services
    .AddMuonroiEfCoreRepositories(config)
    .AddMuonroiDapperQueries(config)
    .AddMuonroiMessagingOutbox(config);
```
...
```powershell
Install-Package Muonroi.Data.Abstractions
```

Alternatively, add it directly to your `.csproj`:

```xml
<PackageReference Include="Muonroi.Data.Abstractions" Version="1.0.0" />
```

## Quick Start

Here is a minimal, complete working example of how to configure and use `Muonroi.Data.Abstractions` in your application.
...
builder.Services.AddMuonroiDataAbstractions(options => 
{
...
namespace SampleApp.Services
{
    public class MyBusinessService
    {
        ...
        {
            _logger.LogInformation("Executing process using Muonroi.Data.Abstractions components.");
...
```csharp
builder.Services.Configure<DataAbstractionsOptions>(
    builder.Configuration.GetSection("Muonroi_DataAbstractions"));
```
...
- `IMuonroiDataAbstractionsService`: The primary contract for interacting with this package. Provides methods to execute subsystem logic.
- `DataAbstractionsOptions`: Configuration model representing the options available for tuning the subsystem.
...
- `muonroi.dataabstractions.operations.count`: Total operations performed.
- `muonroi.dataabstractions.operations.duration`: Histogram of operation latencies in milliseconds.
- `muonroi.dataabstractions.operations.errors`: Counter for exceptions and failures.

### Traces

Traces are emitted under the activity source name `Muonroi.Data.Abstractions`. Span names follow semantic conventions and typically include the operation name.
