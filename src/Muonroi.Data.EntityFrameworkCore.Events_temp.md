...
### + Messaging.MassTransit → Reliable Event Delivery
`MSagaDbContext` persists domain events in the outbox table; MassTransit's relay worker delivers them to the broker exactly-once:
```csharp
builder.Services
    .AddSagaDbContext<MySagaDbContext>(config)
    .AddMassTransit(x => x.AddMuonroiOutbox<MySagaDbContext>());
```

### + Tenancy.Core → Per-Tenant Event Streams
Each tenant's saga state and events are isolated. `ISagaRepository<T>` scopes all queries by `TenantId` automatically.

### + RuleEngine.Core → Rule-Triggered State Transitions
Saga transitions can be gated by rule engine evaluations before persisting the new state.

### Full Event-Sourced Stack
```csharp
builder.Services
    .AddSagaDbContext<MySagaDbContext>(config)
    .AddTenantContext(config)
    .AddMassTransit(x => { x.AddSagas(typeof(Program).Assembly); x.AddMuonroiOutbox<MySagaDbContext>(); })
