<Muonroi.Core>
> Core library implementing fundamental system utilities and context.
...
The `Muonroi.Core` package provides fundamental building blocks for modern .NET applications. It is part of the Muonroi ecosystem, designed to accelerate enterprise software development by providing robust, scalable, and highly cohesive components. 
...
### + Muonroi.Core.Abstractions → Implements the IMDateTimeService, IMJsonSerializeService contracts

Provides the concrete implementations for core infrastructure.

```csharp
builder.Services.AddMuonroiCoreServices(); // Registers IMDateTimeService, etc.
```

### + Muonroi.Tenancy → ISystemExecutionContext carries TenantId + UserId + CorrelationId ambiently

Fuses core context with tenancy, allowing global access to who is doing what.
...
### + Muonroi.Logging → IMLog<T> uses Core's clock for timestamping

Ensures all logs use the synchronized system clock.

```csharp
_logger.LogInformation("System time is strictly managed");
```

### + Muonroi.Diagnostics → Trace sessions use Core's clock for accurate timing

Provides precision timing for diagnostic traces.

```csharp
// Used internally by diagnostics.
```
...
```xml
<PackageReference Include="Muonroi.Core" Version="1.0.0" />
```

## Quick Start

Here is a minimal, complete working example of how to configure and use `Muonroi.Core` in your application.
...
builder.Services.AddMuonroiCore(options => 
{
...
namespace SampleApp.Services
{
    public class MyBusinessService
    {
        ...
        {
            _logger.LogInformation("Executing process using Muonroi.Core components.");
...
```csharp
builder.Services.Configure<CoreOptions>(
    builder.Configuration.GetSection("Muonroi_Core"));
```
...
- `IMuonroiCoreService`: The primary contract for interacting with this package. Provides methods to execute subsystem logic.
- `CoreOptions`: Configuration model representing the options available for tuning the subsystem.
...
- `muonroi.core.operations.count`: Total operations performed.
- `muonroi.core.operations.duration`: Histogram of operation latencies in milliseconds.
- `muonroi.core.operations.errors`: Counter for exceptions and failures.

### Traces

Traces are emitted under the activity source name `Muonroi.Core`. Span names follow semantic conventions and typically include the operation name.
