...
> Distributed Redis caching implementation optimized for the Muonroi ecosystem.
...
The `Muonroi.Caching.Redis` package provides fundamental building blocks for modern .NET applications. It is part of the Muonroi ecosystem, designed to accelerate enterprise software development by providing robust, scalable, and highly cohesive components. 
...
### + Muonroi.Caching.Memory → L1/L2 multi-level: Memory for hot data, Redis for shared state

Combine both packages to achieve extreme performance with shared distributed state.

```csharp
builder.Services.AddMultiLevelCaching();
```

### + Muonroi.Tenancy → Per-tenant Redis key namespacing

Keys are prefixed with the Tenant ID at the cache client level, preventing accidental cross-tenant contamination.

```csharp
// Transparently handled by RedisTenantCache wrapper
```

### + Muonroi.Observability → Redis latency histograms in OTel

Automatically emits timing data for Redis operations to identify latency spikes.

```csharp
builder.Services.AddOpenTelemetry().WithTracing(t => t.AddRedisInstrumentation());
```

### + Muonroi.RuleEngine.Runtime → Distributed rule set cache (multiple pods share the same cache)

Allows multiple application pods to share compiled rules and rule sets.

```csharp
builder.Services.AddRuleEngineRedisCache();
```
...
```xml
<PackageReference Include="Muonroi.Caching.Redis" Version="1.0.0" />
```

## Quick Start

Here is a minimal, complete working example of how to configure and use `Muonroi.Caching.Redis` in your application.
...
builder.Services.AddMuonroiCachingRedis(options => 
{
...
namespace SampleApp.Services
{
    public class MyBusinessService
    {
        ...
        {
            _logger.LogInformation("Executing process using Muonroi.Caching.Redis components.");
...
```csharp
builder.Services.Configure<CachingRedisOptions>(
    builder.Configuration.GetSection("Muonroi_CachingRedis"));
```
...
- `IMuonroiCachingRedisService`: The primary contract for interacting with this package. Provides methods to execute subsystem logic.
- `CachingRedisOptions`: Configuration model representing the options available for tuning the subsystem.
...
- `muonroi.cachingredis.operations.count`: Total operations performed.
- `muonroi.cachingredis.operations.duration`: Histogram of operation latencies in milliseconds.
- `muonroi.cachingredis.operations.errors`: Counter for exceptions and failures.

### Traces

Traces are emitted under the activity source name `Muonroi.Caching.Redis`. Span names follow semantic conventions and typically include the operation name.
