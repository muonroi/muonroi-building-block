...
> Shared utilities, models, exceptions, and extensions used across all other packages.
...
The `Muonroi.BuildingBlock.Shared` package provides fundamental building blocks for modern .NET applications. It is part of the Muonroi ecosystem, designed to accelerate enterprise software development by providing robust, scalable, and highly cohesive components. 
...
### + All Muonroi Packages → Consistent base models and extensions across domains

This is automatically included when you reference most Muonroi packages. It gives you unified exceptions, common enums, and foundational interfaces.

```csharp
// No specific registration required. It provides the types used everywhere.
throw new MuonroiException("Standardized error handling");
```

### Full Ecosystem Stack Example

```csharp
// Automatically used when referencing any other package.
```
...
```powershell
Install-Package Muonroi.BuildingBlock.Shared
```
...
Here is a minimal, complete working example of how to configure and use `Muonroi.BuildingBlock.Shared` in your application.
...
builder.Services.AddMuonroiBuildingBlockShared(options => 
{
...
namespace SampleApp.Services
{
    public class MyBusinessService
    {
        ...
        {
            _logger.LogInformation("Executing process using Muonroi.BuildingBlock.Shared components.");
...
```csharp
builder.Services.Configure<BuildingBlockSharedOptions>(
    builder.Configuration.GetSection("Muonroi_BuildingBlockShared"));
```
...
- `IMuonroiBuildingBlockSharedService`: The primary contract for interacting with this package. Provides methods to execute subsystem logic.
- `BuildingBlockSharedOptions`: Configuration model representing the options available for tuning the subsystem.
...
- `muonroi.buildingblockshared.operations.count`: Total operations performed.
- `muonroi.buildingblockshared.operations.duration`: Histogram of operation latencies in milliseconds.
- `muonroi.buildingblockshared.operations.errors`: Counter for exceptions and failures.

### Traces

Traces are emitted under the activity source name `Muonroi.BuildingBlock.Shared`. Span names follow semantic conventions and typically include the operation name.
