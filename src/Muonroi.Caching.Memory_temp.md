...
> In-memory caching implementation optimized for the Muonroi ecosystem.
...
The `Muonroi.Caching.Memory` package provides fundamental building blocks for modern .NET applications. It is part of the Muonroi ecosystem, designed to accelerate enterprise software development by providing robust, scalable, and highly cohesive components. 
...
### + Muonroi.Caching.Redis → Forms IMultiLevelCacheService: Memory (L1) + Redis (L2)

Combine fast local memory access with a robust distributed backend to reduce load on Redis.

```csharp
builder.Services.AddMultiLevelCaching(options => {
    options.UseMemoryForL1();
    options.UseRedisForL2();
});
```

### + Muonroi.Tenancy.Core → Cache keys automatically include tenant ID

Multi-tenant aware in-memory caching ensures that tenants cannot read each other's data from the L1 cache.

```csharp
app.UseTenantResolution();
// Cache keys are seamlessly prefixed in the background
```

### + Muonroi.Observability → Hit/miss metrics exported to OTel automatically

Automatically record cache hits, misses, and evictions.

```csharp
builder.Services.AddOpenTelemetry().WithMetrics(m => m.AddMeter("Muonroi.Caching"));
```

### + Muonroi.RuleEngine.Core → Rule results can be cached: avoid re-evaluating same inputs

Wrap expensive rule evaluations in a memory cache to drastically improve throughput for frequent conditions.

```csharp
var result = await cache.GetOrCreateAsync("rule_key", entry => ruleEngine.EvaluateAsync(facts));
```
...
```powershell
Install-Package Muonroi.Caching.Memory
```

Alternatively, add it directly to your `.csproj`:

```xml
<PackageReference Include="Muonroi.Caching.Memory" Version="1.0.0" />
```

## Quick Start

Here is a minimal, complete working example of how to configure and use `Muonroi.Caching.Memory` in your application.
...
builder.Services.AddMuonroiCachingMemory(options => 
{
...
namespace SampleApp.Services
{
    public class MyBusinessService
    {
        ...
        {
            _logger.LogInformation("Executing process using Muonroi.Caching.Memory components.");
...
```csharp
builder.Services.Configure<CachingMemoryOptions>(
    builder.Configuration.GetSection("Muonroi_CachingMemory"));
```
...
- `IMuonroiCachingMemoryService`: The primary contract for interacting with this package. Provides methods to execute subsystem logic.
- `CachingMemoryOptions`: Configuration model representing the options available for tuning the subsystem.
...
- `muonroi.cachingmemory.operations.count`: Total operations performed.
- `muonroi.cachingmemory.operations.duration`: Histogram of operation latencies in milliseconds.
- `muonroi.cachingmemory.operations.errors`: Counter for exceptions and failures.

### Traces

Traces are emitted under the activity source name `Muonroi.Caching.Memory`. Span names follow semantic conventions and typically include the operation name.
