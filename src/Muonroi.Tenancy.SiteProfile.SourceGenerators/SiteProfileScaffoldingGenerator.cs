...
public sealed class SiteProfileScaffoldingGenerator : IIncrementalGenerator
{
    ...
    private const string SiteEntityMapAttributeName = "SiteEntityMap";
    ...
    private static ScaffoldingModel? GetScaffoldingModel(GeneratorSyntaxContext context)
    {
        ...
        // Collect [SiteEntityMap] attributes (AllowMultiple = true)
        List<EntityMapModel> entityMaps = new List<EntityMapModel>();
        foreach (var attr in classSymbol.GetAttributes())
        {
            if (attr.AttributeClass is null) continue;
            if (attr.AttributeClass.Name != SiteEntityMapAttributeName &&
                attr.AttributeClass.Name != SiteEntityMapAttributeName + "Attribute")
                continue;
            ...
            entityMaps.Add(new EntityMapModel(
                coreSymbol.ToDisplayString(),
                siteSymbol.ToDisplayString(),
                tableName));
        }
    ...
    private static void Execute(ImmutableArray<ScaffoldingModel?> models, SourceProductionContext context)
    {
        ...
        foreach (var model in models)
        {
            ...
            if (model.EntityMaps.Count > 0)
            {
                string onModelCreatingSource = EmitOnModelCreating(model);
                string onModelCreatingHint = $"{model.ClassName}.OnModelCreating.g.cs";
                context.AddSource(onModelCreatingHint, onModelCreatingSource);
            }
        }
    }
...
";
    ...
    private static string EmitOnModelCreating(ScaffoldingModel model)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated />");
        sb.AppendLine("// Entity hierarchy mapping generated from [SiteEntityMap] attributes.");
        ...
        sb.AppendLine("    /// Applies entity hierarchy mappings to the model builder.");
        sb.AppendLine("    /// Call this from your site DbContext's OnModelCreating override.");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    public static void ApplyEntityHierarchy(ModelBuilder modelBuilder)");
        sb.AppendLine("    {");
        foreach (var map in model.EntityMaps)
        {
            sb.AppendLine($"        modelBuilder.Ignore<{map.CoreTypeName}>();");
            sb.AppendLine($"        modelBuilder.Entity<{map.SiteTypeName}>().ToTable(\"{EscapeStringLiteral(map.TableName)}\");");
        }
        sb.AppendLine("    }");
        sb.AppendLine("}");
        return sb.ToString();
