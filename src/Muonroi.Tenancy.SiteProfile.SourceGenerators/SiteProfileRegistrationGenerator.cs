...
public sealed class SiteProfileRegistrationGenerator : IIncrementalGenerator
{
        ...
        SourceProductionContext context)
    {
        ...
        sb.AppendLine("/// AOT-safe manifest of all discovered ISiteProfile implementations.");
        sb.AppendLine("/// Each profile is instantiated via new() — no Activator.CreateInstance.");
        sb.AppendLine("/// </summary>");
        sb.AppendLine("internal static class SiteProfileManifest");
        sb.AppendLine("{");
        sb.AppendLine("    public static ISiteProfile[] CreateAll() => new ISiteProfile[]");
        sb.AppendLine("    {");

        for (int i = 0; i < profiles.Count; i++)
        {
            string fullName = profiles[i].ToDisplayString();
            sb.AppendLine($"        new {fullName}(),");
        }
        ...
        sb.AppendLine("/// Delegates to SiteProfileManifestRunner.Register() with AOT-safe manifest.");
        ...
        sb.AppendLine("        => SiteProfileManifestRunner.Register(");
        sb.AppendLine("            SiteProfileManifest.CreateAll(), services, configuration, siteCodeAccessor, logFactory);");
        sb.AppendLine("}");
