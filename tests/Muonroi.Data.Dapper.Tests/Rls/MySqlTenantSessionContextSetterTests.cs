...
/// <summary>
/// Tests for <see cref="MySqlTenantSessionContextSetter"/> covering parameterization,
/// empty-string fallback, injection safety, sync/async parity, and OBS-01 logging.
/// </summary>
public sealed class MySqlTenantSessionContextSetterTests
{
    // -------------------------------------------------------------------------
    // Test 1: Enabled + tenant set → one command, CommandText contains @app_current_tenant_id;
    //         @tid parameter == tenant id (not concatenated into CommandText)
    // -------------------------------------------------------------------------

    [Fact]
    public void Apply_WhenTenantSet_ExecutesSetWithAppCurrentTenantId()
    {
        // Arrange
        SpyIMLog<MySqlTenantSessionContextSetter> spy = new();
        MySqlTenantSessionContextSetter sut = new(spy);
        ...
        conn.ExecutedCommands[0].CommandText.Should().Contain("@app_current_tenant_id",
            "MySQL uses a session user-variable @app_current_tenant_id for tenant context");
        conn.ExecutedCommands[0].ParameterValue.Should().Be("tenant-abc",
            "tenant id must be bound as the @tid parameter");
    }
    ...
    public void Apply_WhenTenantNull_ExecutesSetWithEmptyString()
    {
        ...
        conn.ExecutedCommands[0].ParameterValue.Should().Be(string.Empty,
            "null tenant id must map to empty string so views block all rows downstream");
    }
