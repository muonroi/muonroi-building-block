...
{
    private readonly MUpgradeCompatibilityService _service = new(Substitute.For<IMLog<MUpgradeCompatibilityService>>());
    ...
    public void Evaluate_ConfigSignedPolicyDisabled_ReturnsBlockingIssue()
    {
        MUpgradeCompatibilityRequest request = new()
        {
            BaselineConfig = new MUpgradeLicenseConfigSnapshot { RequireSignedPolicy = true },
            TargetConfig = new MUpgradeLicenseConfigSnapshot { RequireSignedPolicy = false }
        };
    ...
    [Fact]
    public void Evaluate_MajorVersionJump_WithTreatWarningsAsBlocking_ShouldBecomeIncompatible()
    {
        MUpgradeCompatibilityRequest request = new()
        {
    ...
    private sealed class CompatibilityFileHarness : IDisposable
    {
        ...
        {
            _root = Path.Combine(Path.GetTempPath(), "muonroi-enterprise-upgrade-tests", Guid.NewGuid().ToString("N"));
