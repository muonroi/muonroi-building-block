...
public class MediatorTests
{
    ...
    {
        public async Task<MResponse> Handle(TRequest request, RequestHandlerDelegate<MResponse> next,
            CancellationToken cancellationToken)
        {
            log.Add("before");
            MResponse? resp = await next(cancellationToken);
            log.Add("after");
            return resp;
        }
    }
    ...
    {
        public async IAsyncEnumerable<int> Handle(NumberStreamRequest request,
    ...
    public async Task CreateStream_Returns_IAsyncEnumerable()
    {
        ...
        List<int> result = [];
        await foreach (int i in mediator.CreateStream(new NumberStreamRequest(3))) result.Add(i);
