...
public class MDateTimeConverterTests
{
    ...
    public void Read_Parses_Valid_String()
    {
        ...
        _ = reader.Read();
        MDateTimeConverter conv = new();
        DateTime result = conv.Read(ref reader, typeof(DateTime), new JsonSerializerOptions());
        Assert.Equal(
            DateTime.Parse(text, CultureInfo.InvariantCulture,
                DateTimeStyles.AdjustToUniversal | DateTimeStyles.AssumeUniversal), result);
    ...
    public void Read_Invalid_Format_Throws()
    {
        Assert.ThrowsAny<JsonException>(() =>
        {
            ...
            Utf8JsonReader r = new(data);
            _ = r.Read();
            MDateTimeConverter conv = new();
            conv.Read(ref r, typeof(DateTime), new JsonSerializerOptions());
        });
    }
    ...
    public void Write_Serializes_As_Utc()
    {
        ...
        conv.Write(writer, dt, new JsonSerializerOptions());
        writer.Flush();
        string json = Encoding.UTF8.GetString(ms.ToArray());
        string expected = "\"" + dt.ToString("O") + "\"";
