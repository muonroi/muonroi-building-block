...
public class PermissionServiceBuildTreeTests
{
    ...
    {
        List<string> names = [];
        for (int i = 0; i < 1000; i++) names.Add($"Menu{i % 10}.Action{i}");
    ...
    {
        PermissionService<TestPerm, TestDbContext> svc = new(null!, null!, null!);
        Assert.NotNull(svc);
    }
    ...
    public void Constructor_With_Valid_Dependencies()
    {
        ...
        PermissionService<TestPerm, TestDbContext> svc = new(db, ctx, new FakeDateTimeService());
        Assert.NotNull(svc);
    }
}
