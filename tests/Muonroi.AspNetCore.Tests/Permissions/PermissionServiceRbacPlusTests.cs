...
public class PermissionServiceRbacPlusTests
{
    ...
    public async Task AssignRoleToUserAsync_Invalidates_UserPermissionCache()
    {
        ...
        PermissionService<TestPerm, TestDbContext> service = new(db, ctx, new FakeDateTimeService(), cache, new TestLicenseGuard());
    ...
    public async Task AdvancedAuthFeature_NotLicensed_ShouldThrow()
    {
        ...
        PermissionService<TestPerm, TestDbContext> service = new(db, ctx, new FakeDateTimeService(), null, new DenyAdvancedAuthGuard());
