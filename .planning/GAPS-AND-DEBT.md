...
| G18 | `<h2>` not bold + `text-transform:uppercase` not applied to `<h2>` text in CHNG_E header | 8.12b (Chrome-MCP visual diff post-G15b) | OPEN — UA stylesheet missing `h2 { font-weight:bold }`; class-rule lookup doesn't cover `text-transform` | 8.13 |
| G19 | `<p>label: <strong>value</strong></p>` wraps to 2 lines inside `div.w-30.float-left` despite ample content-box width | 8.12b (post-G15b visual diff) | OPEN — float content-box width may be shrink-to-fit min-content rather than honoring class-rule `width:30%` | 8.13 |
| G20 | `<th style="width:16%">` wraps each word as own line — column appears 1-char wide | 8.12b (post-G15b visual diff) | OPEN — inline `%` width on `<th>` not propagating to table column-width solver (G15 covers class-rule path; this is inline-style path on table cells) | 8.13 |
| G21 | `<p>Số điện thoại: 0901234567</p>` wraps 3 lines inside `div.w-25.float-left.text-center` | 8.12b (post-G15b visual diff) | OPEN — likely same root-cause family as G19 (% width on nested float not class-resolved) | 8.13 |
