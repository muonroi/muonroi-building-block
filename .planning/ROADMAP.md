...
Nine phases deliver a pure-managed HTML/CSS-to-PDF renderer from zero to enterprise-grade. Phases 1â€“7 ship the v0.1 OSS engine: public contracts first, then pipeline stages in strict dependency order (parse â†’ cascade â†’ layout â†’ font/image â†’ write), locked by a golden snapshot corpus and CI gates before any NuGet publish. Phase 8 hardens v0.2 with compile-time source generation, AOT trim-safety, and allocation reduction. Phase 9 builds the commercial Enterprise tier on the stable OSS core.

## Phases

- [x] **Phase 1: Abstractions + Contracts** â€” Define all public API contracts and adapter seams in `Muonroi.Pdf.Abstractions` (netstandard2.0); zero implementation code (completed 2026-05-26)
- [x] **Phase 2: Parse + Cascade + Policy Gate** â€” Wire AngleSharp HTML parsing, AngleSharp.Css cascade, and `IPdfCssPolicy.DefaultStrict` in `Muonroi.Pdf.Governance` (completed 2026-05-26)
- [x] **Phase 3: Box Tree + Layout Engine** â€” Hand-written box tree with block/inline formatting, table layout, pagination, and page counters (completed 2026-05-26)
- [x] **Phase 4: Font + Image Pipeline** â€” `IFontResolver` integration, Vietnamese diacritic shaping via SixLabors.Fonts, PNG/JPEG/data-URI image decoding (completed 2026-05-27)
...
**Success Criteria** (what must be TRUE):
  1. `Muonroi.Pdf.Abstractions` compiles targeting `netstandard2.0` with zero implementation code â€” interfaces and records only
  ...
  4. `PdfRenderResult` carries metadata only (`PageCount`, `ByteCount`, `Elapsed`, `TemplateHash`, `PolicyId`, `Diagnostics`); PDF bytes are written directly to the caller-supplied `Stream destination` on `IMPdfService.RenderAsync` â€” no content buffering on the result type
...
Plans:
- [x] 01-01-PLAN.md â€” Fix csproj (netstandard2.0), GlobalUsings, CPM version pins; add PdfConfigs, PdfRenderResult.Diagnostics, PdfTelemetryNames
- [x] 01-02-PLAN.md â€” Create Engine/ adapter seams: 4 marker types + 4 seam interfaces (IHtmlParser, ICssCascadeEngine, IImageDecoder, IPdfWriter)
- [x] 01-03-PLAN.md â€” Create Muonroi.Pdf.Enterprise stub (PKG-04) + build verification for both projects
- [x] 01-04-PLAN.md â€” Gap closure: commit untracked public API contracts, fix build warnings, sync REQUIREMENTS + ROADMAP to implemented signatures
...
Plans:
- [x] 02-01-PLAN.md â€” Abstractions gap closure: exception hierarchy (PdfException, PdfInputLimitException, PdfPolicyException), extended PolicyViolation, RequirePolicySignature on PdfConfigs
- [x] 02-02-PLAN.md â€” Create Muonroi.Pdf.Governance csproj (net8.0) + test project scaffold + register both in solution
- [x] 02-03-PLAN.md â€” HTML parsing adapter: AngleSharpParsedDocument + AngleSharpHtmlParser with PIPE-01/PIPE-02 limit enforcement
- [x] 02-04-PLAN.md â€” CSS cascade adapter: AngleSharpStyledDocument + AngleSharpCascadeEngine with IPdfDocumentContext eager metrics
- [x] 02-05-PLAN.md â€” Policy gate: DefaultStrictPolicy (GOV-01/GOV-02, 9 blocked features) + SignedPdfCssPolicyDecorator (GOV-03)
...
**Success Criteria** (what must be TRUE):
  1. Adjacent block elements with vertical margins collapse to the maximum margin per CSS 2.1 Â§8.3.1; BFC roots (overflow:hidden, table cells) do not collapse across the root boundary
...
Plans:
- [x] 03-01-PLAN.md â€” Abstractions contracts (IStyledNode/IComputedStyle/IPageRule) + Muonroi.Pdf csproj setup
- [x] 03-02-PLAN.md â€” Governance gap: AngleSharpStyledNode/ComputedStyle/PageRule + extend AngleSharpStyledDocument
- [x] 03-03-PLAN.md â€” Box tree types: geometry helpers + full BoxNode hierarchy (12 files)
- [x] 03-04-PLAN.md â€” BoxTreeBuilder + ITextMetrics seam + positioning types + test project scaffold
- [x] 03-05-PLAN.md â€” BlockLayoutEngine (BFC, margin collapsing) + InlineLayoutEngine (IFC, baseline)
- [x] 03-06-PLAN.md â€” TableLayoutEngine (colspan/rowspan) + PaginationEngine (breaks, counters, header/footer)
- [x] 03-07-PLAN.md â€” LayoutEngine two-pass entry point + KNOWN-DEVIATIONS.md
- [x] 03-08-PLAN.md â€” Unit tests SC1â€“SC5 (22 tests, dotnet test exits 0)
- [x] 03-09-PLAN.md â€” Gap closure: LAYOUT-07 border-collapse policy fix + governance test + KD-03-05 Vietnamese break test
...
**Success Criteria** (what must be TRUE):
  ...
  2. Vietnamese text "Tiáº¿ng Viá»‡t" renders with correctly stacked diacritics â€” no replacement glyphs, correct combining-mark positions above base glyphs
...
Plans:
- [x] 04-01-PLAN.md â€” FontFaceDeclaration + IStyledDocument.FontFaces (Abstractions) + AngleSharpStyledDocument implementation (Governance)
- [x] 04-02-PLAN.md â€” SixLabors.Fonts csproj ref + EmbeddedFontInfo + SixLaborsTextMetrics + GlyphCollector
- [x] 04-03-PLAN.md â€” DataUriDecoder + PureImageDecoder (PNG IHDR + JPEG SOF) + ImagePipeline async pre-pass
- [x] 04-04-PLAN.md â€” TrueTypeFontSubsetter (TTF binary subsetter) + FontPipeline async orchestrator
- [x] 04-05-PLAN.md â€” PositionedPageList extended + BoxTreeBuilder resolvedImages + LayoutEngine.LayoutAsync wiring
- [x] 04-06-PLAN.md â€” Unit tests: FontPipelineTests, VietnameseDiacriticTests, ImagePipelineTests, TrueTypeFontSubsetterTests
...
Plans:
- [x] 05-01-PLAN.md â€” PdfSecurityException + ThrowingResourceResolver + <script> policy rejection + PdfSharpCore csproj ref
- [x] 05-02-PLAN.md â€” PdfSharpFontResolverAdapter + PdfSharpCoreWriter (text, image, font, determinism, security hardening)
- [x] 05-03-PLAN.md â€” Tests: PdfWriterTests, DeterminismTests, SecurityTests (â‰¥16 new tests)
...
**Success Criteria** (what must be TRUE):
  ...
  5. `PdfConfigs` bound from `"PdfConfigs"` IConfiguration section with `MaxPages: 0` throws a validation exception at startup â€” before any render is attempted
**Plans**: TBD
...
**Success Criteria** (what must be TRUE):
  ...
  3. Cold render of the 50 KB reference template completes in â‰¤300 ms; warm render in â‰¤80 ms on a single developer-machine thread
...
Plans:
- [x] 07-01-PLAN.md â€” Golden harness (hand-rolled byte comparer) + corpus registry + determinism canary + block-layout first batch + .gitattributes
- [x] 07-02-PLAN.md â€” Remaining structural corpus (inline/table/paged-media/image/font/security) to reach 40+ cases
- [x] 07-03-PLAN.md â€” Vietnamese corpus (10+) + glyph-coverage guard + finalize KNOWN-DEVIATIONS.md
- [x] 07-04-PLAN.md â€” Informational perf gate (cold/warm, generous CI ceiling, skippable, SlowIntegration)
- [ ] 07-05-PLAN.md â€” Gate scripts green + pack 4 .nupkg at 1.0.0-alpha.N + PKG-05 meta-package & PKG-06 OSS-BOUNDARY

### Phase 8: v0.2 â€” Source Generator + AOT + DesignSystem
...
**Success Criteria** (what must be TRUE):
  ...
  2. Source-generated warm throughput on the 50 KB template is â‰¥3Ã— the v0.1 runtime factory baseline (BenchmarkDotNet)
...
**Success Criteria** (what must be TRUE):
  ...
  5. At least 3 paid Enterprise customers are active and ARR is â‰¥$60k at v1.0 GA
**Plans**: TBD
**UI hint**: yes
