...
CURRENT_DIR="${ROOT_DIR}/schema/antitamper/contracts"
BASELINE_DIR="${ROOT_DIR}/schema/antitamper/baseline"
...
if not current_files:
    raise SystemExit("No Anti-Tampering contract files found.")
...
for current_path in current_files:
    ...
    if current_version < baseline_version:
        errors.append(f"[BREAKING] {name}: version regressed {current_version} < {baseline_version}")
    ...
    if removed_required:
        errors.append(f"[BREAKING] {name}: required fields removed: {', '.join(removed_required)}")
    ...
    for prop, baseline_def in baseline_props.items():
        ...
        if baseline_type != current_type:
            errors.append(
                f"[BREAKING] {name}: property '{prop}' type changed from '{baseline_type}' to '{current_type}'"
            )
...
print("Anti-Tampering contract governance check passed.")
PY
