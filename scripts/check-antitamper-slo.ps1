...
Write-Host "Current Anti-Tampering metrics: p95=$($current.p95Ms)ms, errorRate=$($current.errorRate), tamperDetections=$($current.tamperDetections)"
...
if (-not [string]::IsNullOrWhiteSpace($BaselineMetricsPath)) {
    ...
    Write-Host "Baseline Anti-Tampering metrics: p95=$($baseline.p95Ms)ms, errorRate=$($baseline.errorRate), tamperDetections=$($baseline.tamperDetections)"
...
Write-Host "Anti-Tampering SLO check passed."
